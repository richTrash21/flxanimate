...
class AnimationData
{
	...
	public static function fromColorJson(effect:ColorEffects = null)
	{
		...
		{
			case Tint, "Tint":
				colorEffect = Tint(Std.parseInt("0x" + effect.TC.substring(1)), effect.TM);
			...
			case Advanced, "Advanced":
				colorEffect = Advanced(new ColorTransform(effect.RM, effect.GM, effect.BM, effect.AM,
	...
	public static function parseColorEffect(colorEffect:ColorEffect = None, ?CT:ColorTransform)
	{
		if (colorEffect != null)
		{
			...
			switch (colorEffect)
			{
				...
				case Brightness(brightness):
					CT.redMultiplier = CT.greenMultiplier = CT.blueMultiplier -= Math.abs(brightness);
					if (brightness >= 0)
						CT.redOffset = CT.greenOffset = CT.blueOffset = 255 * brightness;
...
abstract ColorEffects({}) from {}
{
	...
	function get_BRT()
	{
		return AnimationData.setFieldBool(this, ["BRT", "Brightness", "brightness"]);
	}
}
